...
import { get_mines, disarm_mine } from "./scripts/api_service";
...
start_btn.addEventListener("click", async (e) => {
  e.preventDefault();
  if (begin === true && exploration_run === false) {
    exploration_run = true;
    await begin_exploration();
  } else if (exploration_run === true)
    alert(
      "An exploration process is already running, please wait till it finished"
    );
...
async function begin_exploration() {
  ...
  for (let i = 0; i < commands.length; i++) {
    if (commands.at(i) === "L" || commands.at(i) === "R") {
      ...
      if (update === true) {
        ...
        } else update_cell(curr_row, curr_col, move_num);
      }
    }
  }
  ...
  for (const row of map_elem.children) {
    for (const cell of row.children) {
      cell.textContent = cell.textContent.substring(
        0,
        cell.textContent.length - 2
      );
    }
  }
